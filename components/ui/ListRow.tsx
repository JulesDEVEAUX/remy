...
export function ListRow({
  ...
  href: string;
  icon?: IconName;
  /** Emoji choisi par le foyer (produit, recette — cf. issue #73) : prime sur `icon` si fourni. */
  emoji?: string;
  label: string;
  meta?: string;
  tag?: ReactNode;
  ...
  return (
    ...
    >
      {emoji ? (
        <span className="flex size-6 flex-none items-center justify-center text-[18px]" aria-hidden>
          {emoji}
        </span>
      ) : (
        icon && (
